...
class PlotPlatformForcesWidget(QtWidgets.QWidget):
    ...
    def update(self, timestamp_list: list, forces_data: dict = None):
        ...
        fx_values = [np.array(
            forces_data[key]) * -1 if '1' in key or '4' in key else np.array(forces_data[key]) for key in fx_keys]
        # Invert signs when label is 1 or 2
        fy_values = [np.array(
            forces_data[key]) * -1 if '1' in key or '2' in key else np.array(forces_data[key]) for key in fy_keys]
        ...
        time_incr_np = np.array([(t - timestamp_list[0])/1000
...
class PlotPlatformCOPWidget(QtWidgets.QWidget):
    ...
    def update(self, cop_list: list = None):
        ...
        self.ax.plot(cop_x[-last_indexes], cop_y[-last_indexes],
